...
public class MakingChessBoardsFailLastTwoLargePracticeTests {
	...
	private static String doit(boolean[][] board) {
		...
		int bestSize, bestRow=0, bestCol=0, lastBestSize = 0;
		do {
			bestSize=0;
			loop:
			for (int i=0;i<rows; i++) {
			...
			if (bestSize > 0) {
				boardSizes[bestSize] ++;
				for (int i=bestRow; i<bestRow+bestSize; i++) {
					for (int j=bestCol; j<bestCol+bestSize; j++) {
						visited[i][j] = true;
					}
				}
	...
	private static Object printBoard(boolean[][] board) {
		StringBuilder strAux = new StringBuilder();
		int rows = board.length;
		int cols = board[0].length;
		for (int i=0;i<rows; i++) {
			for (int j=0; j<cols; j++) {
				strAux.append(board[i][j] ? "1":"0");
			}
			strAux.append('\n');
		}
		return strAux.toString();
